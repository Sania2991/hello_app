...
class UserTest < ActiveSupport::TestCase
  ...
  test "email should not be too long" do
    @user.email = "a"*244 + "@example.com"   #не >255
    assert_not @user.valid?
  end
  ...
  test "email validation shoul reject invalid addresses" do
    invalid_addresses = %w[user@example,com user_at_foo.org user.name@example.
                           foo@example,com foo@bar+baz.com foo@bar..com]
  ...
  test "authenticatid? should return false for a user with nil digest" do
    assert_not @user.authenticated?(:remember, '')
  end

  # Если удаляем польз. => удал. и сообщ.
  ...
  # Тесты вспомогательных мет. follow/unfollow/following?
